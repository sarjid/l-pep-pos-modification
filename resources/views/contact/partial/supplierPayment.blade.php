@php
$supplier_due_amount = round($contact->total_purchase - ($contact->total_normal_paid_amount + $contact->total_purchase_paid_amount), 2);
$business_due_amount = round($contact->total_purchase_return - $contact->total_purchase_return_paid_amount, 2);
@endphp
<div class="text-center" style="margin-top: -16px">
    <h4 style="font-size: 29px">Supplier Payment</h4>
    <hr>
</div>
...
<div style="margin-top: -5px">
    <form method="POST" action="{{ route('contact.payment.store') }}">
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="well">
                    <strong>Total Purchase: </strong>{{ $contact->total_purchase }} BDT</span><br>
                    <strong>Total Paid: </strong> {{ $contact->total_purchase_paid_amount }} BDT</span><br>
                    <strong>Supplier Due: </strong> {{ $supplier_due_amount }} BDT</span><br>
                    <strong>Business Due: </strong> {{ $business_due_amount }} BDT</span><br>
                    <strong>Payable Amount: </strong> {{ $supplier_due_amount - $business_due_amount }} BDT</span><br>
                </div>
            </div>
        </div>
        ...
        @php
            $is_pay_amount = $supplier_due_amount - $business_due_amount > 0;
        @endphp
        @if ($is_pay_amount)
