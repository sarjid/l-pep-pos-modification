    ...
    <div class="row">
        <div class="col-12">
            <div class="card-box table-responsive mt-4" style="border-top: 3px solid #2aa9b9;">
                <div class="row">
                    <div class="col-md-6">
                        <h4 class="m-t-0 header-title mb-5"><b>{{ __('page.calf-birth-problem')[0] }}</b></h4>
                    </div>
                    <div class="col-md-6 text-right">
                        @if (permission('uni2'))
                            <button class="btn btn-primary waves-effect waves-light m-b-5" id="addNew"><i
                                    class="fa fa-plus-square m-r-5"></i> <span>{{ __('page.calf-birth-problem')[1] }}</span>
                            </button>
                        @endif
                    </div>
                </div>
                ...
                <table id="data-table" class="table table-striped table-bordered mt-4" cellspacing="0" width="100%">
                    ...
                    <tr>
                        <th>{{ __('page.calf-birth-problem')[2] }}</th>
                        <th>{{ __('page.calf-birth-problem')[3] }}</th>
                        <th>{{ __('page.calf-birth-problem')[4] }}</th>
                    </tr>
                    </thead>
                </table>
            </div>
        ...
        $(document).ready(function () {
            ...
            function getData() {
                $('#data-table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('calf-birth-problems.json.all') }}",
                    order: [ [0, 'desc'] ],
                    columns: [
                        {
                            width: '8%',
            ...
            $('body').on('click', "#addNew", function () {
                $.get("{{ route('calf-birth-problems.create') }}", function (data) {
            ...
            $('body').on('click', "#submit", function (e) {
                ...
                $.ajax({
                    url: "{{ route('calf-birth-problems.store') }}",
                    data: {
            ...
            $('body').on('click', "#submitUpdate", function (e) {
                ...
                $.ajax({
                    url: "{{ route('calf-birth-problems.update', '#id') }}".replace('#id', id),
                    data: {
                        _token: "{{ csrf_token() }}", user_id: '{{auth()->id()}}',
            ...
            $('body').on('click', "#tableEdit", function () {
                ...
                $.get('{{route('calf-birth-problems.edit', '#id')}}'.replace('#id', id), function (data) {
                    $('#modalContent').html(data)
                    $("#tableModal").modal('show')
                });
            })
            ...
            $('body').on('click', "#deleteData", function () {
                ...
                swal({
                    ...
                    .then((willDelete) => {
                        if (willDelete) {
                            $.ajax({
                                url: "{{ route('calf-birth-problems.destroy', '#id') }}".replace('#id', id),
