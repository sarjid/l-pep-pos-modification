<?php

namespace App\Http\Controllers\App;

use App\Models\CattleFood;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Request;
use App\Http\Controllers\Controller;

class CattleFoodController extends Controller
{
    public function index()
    {
        return view('cattle-foods.index');
    }

    public function create()
    {
        return view('cattle-foods.create');
    }

    public function store(Request $request): JsonResponse
    {
        CattleFood::query()
            ->create([
                'name' => $request->name,
                'unit' => $request->unit,
                'created_by' => $request->user_id,
            ]);

        return response()->json('Food saved successfully!');
    }

    public function edit($id)
    {
        $cattleFood = CattleFood::query()->findOrFail($id);

        return view('cattle-foods.edit', compact('cattleFood'));
    }

    public function update(Request $request, $id): JsonResponse
    {
        CattleFood::query()->where('id', $id)
            ->update([
                'name' => $request->name,
                'unit' => $request->unit,
                'updated_by' => $request->user_id,
                'updated_at' => date('Y-m-d H:i:s'),
            ]);

        return response()->json('Food updated successfully!');
    }

    public function destroy($id): JsonResponse
    {
        CattleFood::query()->where('id', $id)->delete();

        return response()->json('Food deleted successfully!');
    }

    public function allAsJson()
    {
        return datatables()->of(
            CattleFood::query()
                ->with('createdBy', 'updatedBy')
        )
            ->addColumn('action', function ($data) {
                $btn = "<div class='btn-group'>" . $data->log();
                if (permission('uni3')) {
                    $btn .= '<a data-id="' . $data->id . '" class="btn btn-primary btn-sm" id="tableEdit">Edit</a>';
                }
                if (permission('uni4')) {
                    $btn .= ' <a href="javascript:void(0)" data-toggle="tooltip"  data-id="' . $data->id . '" id="deleteData" class="btn btn-danger btn-sm">Delete</a>';
                }
                return $btn;
            })
            ->rawColumns(['action'])
            ->addIndexColumn()
            ->make(true);
    }
}
