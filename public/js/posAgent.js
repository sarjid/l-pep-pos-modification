...
function productShowInTable(result) {
    ...
    $("input[name=customer_id]").val($("#app_customer_id").val());
...
$("#payment").on("click", function () {
    const payingAmount = $("input[name=paying_amount]").val();
    const availableAmount = $("#customer_points").val();
    const finalAmount = Number(payingAmount);
    $("input[name=customer_id]").val($("#app_customer_id").val());

    // let shouldDisable = false;

    // if (Number($("#app_customer_id").val()) < 1) {
    //     shouldDisable = true;
    // } else if (availableAmount <= 0) {
    //     shouldDisable = true;
    // } else if (finalAmount == 0 || finalAmount > availableAmount) {
    //     shouldDisable = true;
    // }

    // $("#checkout").attr("disabled", shouldDisable);
});

$("#app_customer_id").on("change", function () {
    $.get(
        ROUTE_CUSTOMER_AVAILABLE_POINTS.replace("0", $(this).val()),
        function (res) {
            $("#customer_points").val(res.points);
        }
    );
});
...
$(document).on("change", "select.batch_id", function () {
    $.get(
        `${ROUTE_CART_CHANGE_BATCH_ID}?id=${$(this)
            .closest("tr")
            .data("sc-id")}&stock_transfer_detail_id=${$(this).val()}`,
        function (res) {
...
$(".open-payment-modal").click(function () {
    var cusSelection = $('#contact_id').find(":selected");
